"""
Training script for TGVAnn on KSCI Sugarcane Dataset
...
def plot_training_history(history, save_path='training_history_sugarcane.png'):
    ...
    ax1.set_title('KSCI Sugarcane - Training and Validation Loss', fontsize=14, fontweight='bold')
    ax1.legend()
    ...
    ax2.set_title('KSCI Sugarcane - Training and Validation Accuracy', fontsize=14, fontweight='bold')
    ax2.legend()
...
def main(args):
    ...
    print("\n" + "="*80)
    print("TGVAnn Training - KSCI Sugarcane Dataset")
    print("5 Classes: Healthy, Mosaic, RedRot, Rust, Yellow")
    print("="*80)
    
    # Prepare data
    print("\nPreparing KSCI Sugarcane datasets...")
    ...
    # Create model
    print("\nCreating TGVAnn model for 5-class classification...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            best_model_path = os.path.join(args.output_dir, 'best_model_sugarcane.pth')
            save_model_checkpoint(model, optimizer, epoch, val_loss, val_acc, best_model_path)
            print(f"✓ New best model saved! Accuracy: {val_acc:.2f}%")
        
        # Save checkpoint every N epochs
        if epoch % args.save_freq == 0:
            checkpoint_path = os.path.join(args.output_dir, f'sugarcane_checkpoint_epoch_{epoch}.pth')
            save_model_checkpoint(model, optimizer, epoch, val_loss, val_acc, checkpoint_path)
    
    # Save final model
    final_model_path = os.path.join(args.output_dir, 'final_model_sugarcane.pth')
    save_model_checkpoint(model, optimizer, args.epochs, val_loss, val_acc, final_model_path)
    
    # Plot training history
    plot_path = os.path.join(args.output_dir, 'training_history_sugarcane.png')
    plot_training_history(history, plot_path)
    
    print(f"\n{'='*60}")
    print("Training Complete - KSCI Sugarcane Dataset!")
    print(f"{'='*60}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train TGVAnn on KSCI Sugarcane Dataset')
    
    # Data parameters
    parser.add_argument('--rgb_dir', type=str, default='./Sugarcane_RGB', 
                       help='Path to Sugarcane RGB dataset')
    parser.add_argument('--texture_dir', type=str, default='./Sugarcane_Texture',
                       help='Path to Sugarcane texture dataset')
    parser.add_argument('--output_dir', type=str, default='./outputs/sugarcane', 
    ...
    parser.add_argument('--batch_size', type=int, default=16, 
                       help='Batch size (16 recommended for 5-class)')
    parser.add_argument('--epochs', type=int, default=100, 
                       help='Number of epochs (100 recommended for Sugarcane)')
    parser.add_argument('--lr', type=float, default=1e-5, 
                       help='Learning rate (1e-5 recommended for Sugarcane)')
    ...
    parser.add_argument('--save_freq', type=int, default=20, help='Save checkpoint every N epochs')
