"""
Complete End-to-End Example Workflow for TGVAnn
Demonstrates: Data preparation → Training → Evaluation → Visualization

This script shows a minimal working example for Maize dataset (TMCI)
"""
...
from models import TGVAnn, model_summary, count_parameters
...
def step1_prepare_texture_data():
    ...
    print("\n" + "="*80)
    print("STEP 1: Preparing Texture Data")
    print("="*80)
    
    input_folder = "Maize_RGB"  # Your RGB dataset folder
    output_folder = "Maize_Texture"  # Output texture folder
    ...
    if os.path.exists(output_folder):
        ...
        process_dataset(
            ...
            texture_type='single',  # Single-channel grayscale texture
...
def step2_create_model(num_classes=3):
    """Step 2: Create and analyze TGVAnn model"""
    print("\n" + "="*80)
    print("STEP 2: Creating TGVAnn Model")
    print("="*80)
    ...
    # model_summary(model, device='cuda' if torch.cuda.is_available() else 'cpu')
    
    return model


def step3_prepare_data(rgb_dir, texture_dir, batch_size=32):
...
def step4_train_model(model, train_loader, val_loader, num_epochs=5):
    """Step 4: Train the model (simplified version)"""
    print("\n" + "="*80)
...
def main():
    """Main workflow"""
    print("\n" + "="*80)
    print("TGVAnn Complete Workflow Example")
    print("Maize Dataset (TMCI) - Crop Leaf Disease Detection")
    print("="*80)
    
    # Configuration
    NUM_CLASSES = 3
    BATCH_SIZE = 32
    NUM_EPOCHS = 5  # Increase for better results (e.g., 50)
    ...
    try:
        ...
        print("="*80)
        print("\nNext Steps:")
        print("1. Train for more epochs (--epochs 50) for better results")
        print("2. Generate Grad-CAM visualizations: python visualize_gradcam.py")
        print("3. Evaluate on separate test set if available")
        print("4. Experiment with hyperparameters (learning rate, batch size)")
